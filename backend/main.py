...
auth_token = 'c7129f19f6060a768cf33743967e2cad'
...
def send_otp():
    ...
    try:
        message = client.messages.create(
            ...
            to='+917993186148'  # Hardcoded mobile number
        )
...
def generate_renewable_plot():
    ...
    for item in a:
        ...
        if len(split_values) >= 2:
            start, end = split_values[:2]  # Take the first two values
            result.append(start)
        else:
            # Handle the case when there are not enough values after splitting
